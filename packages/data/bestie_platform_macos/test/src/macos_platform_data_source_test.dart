...
FakePlatform _platform({required String resolvedExecutable}) => FakePlatform(
  ...
  environment: const {'HOME': '/Users/tester', 'TMPDIR': '/var/folders/ab/T/'},
  numberOfProcessors: 8,
  pathSeparator: '/',
);

MacOSPlatformDataSource _dataSource(
  MemoryFileSystem fs, {
...
void main() {
  ...
  test('resolves the temp directory TMPDIR names, sans trailing slash', () {
    final fs = MemoryFileSystem.test();
    _createAssets(fs, _mirror(fs, bundled: false));

    final platform = _dataSource(fs, bundled: false).loadPlatform();

    expect(platform.tempDir, '/var/folders/ab/T');
  });
